...
cat mercury.template.yaml | DOLLAR='$' envsubst > docker-compose.yml
...
docker stop `docker ps -aq -f name=mercury_*`
docker rm `docker ps -aq -f name=mercury_*`
docker network prune -f
docker-compose -p "mercury" up -d
